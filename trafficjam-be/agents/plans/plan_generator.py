...
def _find_nearby_shopping(
    home: Building, buildings: list[Building], max_dist: float = 2000
) -> Building | None:
...
def generate_plan_adult_dropoff_work(adult: Adult) -> DailyPlan:
    child = adult.children[0]  # First child with school
    mode = _get_mode(adult)
    plan = DailyPlan()

    # we already checked it before calling the function
...
def generate_plan_child(child: Child) -> DailyPlan | None:
    if child.needs_dropoff or not child.school or child.age < 12:
        return None
