...
def find_nearby_or_closest(
    ...
    candidates = [
        b
        for b in buildings
        if lat_min <= b["position"][0] <= lat_max
        and lon_min <= b["position"][1] <= lon_max
    ]
    ...
    if candidates:
        within_radius = [
            b
            for b in candidates
            if haversine_distance(agent_pos, b["position"]) <= radius
        ]
        if within_radius:
...
def assign_amenity_preferences(
    agent: Dict, buildings: List[Dict], radius: float = DEFAULT_AMENITY_RADIUS
) -> None:
